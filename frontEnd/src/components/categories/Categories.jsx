...
const Categories = () => {
  return (
    <article id="categories" className="mt-20">
      ...
      <Link to="book?category=romance" className="grid grid-cols-4 gap-5 max-md:grid-cols-3 max-sm:grid-cols-2">
        {CategoriesList.map((item, index) => {
          return (
            <div key={index} className="relative cursor-pointer">
              <img src={item.picture} alt="" className="rounded-md" />
              <h3 className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#00000065] w-full text-center text-lg py-5 max-md:text-base max-sm:text-xs">{item.name}</h3>
            </div>
          );
        })}
      </Link>
